...
//             this.dictOptions = res.result;
            this.getDictOptions(res.result)
          }
        })
      },
      handleInput(e) {
      ...
      getDictOptions(options){
        ...
        console.log('excludeValues',this.excludeValues)
        if(this.excludeValues && this.excludeValues.length > 0){
          for (let i=0;i< options.length;i++){
            let flag = true;
            for(let j=0;j<this.excludeValues.length;j++){
              if(options[i].value === this.excludeValues[j]){
                flag = false;
                break
              }
            }
            if(flag){
              this.dictOptions.push(options[i])
            }
          }
        }else {
