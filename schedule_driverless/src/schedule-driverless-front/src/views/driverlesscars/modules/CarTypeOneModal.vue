<template>
    <section id="section">
        <div class="con-head">
            <div>排班计划（当前为 <span>预设计划</span>，最后一次生成计划时间为2023-10-17 12:30:00）</div>
            <div>总（趟次:80 /援10 派车:43/援10 里程564km 工时98h ）</div>
            <div class="time-choice">
                <img v-if="playBool" src="@/assets/driverlesscars/play.png" alt="" />
                <img v-else src="@/assets/driverlesscars/stop.png" alt="" />
                <a-time-picker :value="time" />
            </div>
                                            ...
                                            <div
                                                ...
                                                :style="{
                                                    background: `conic-gradient(
                                                                ...
                                                                #b6b6b6 ${
                                                                    item.cars ? item.cars.fullLoadRatio : 0
                                                                }% 100%,
                                                                #b6b6b6 100% 100%
                                                            )`
                                                }"
            ...
            <div class="in-car" :class="`type${carType}`">
                <div class="content">
                    ...
                    <div class="car-box">
                        ...
                        <div class="bottom-st">{{ arrUpTwo[arrUpTwo.length - 1].routeStationName }}</div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        getListByRouteId() {
            let that = this;
            console.log('stationOne', stationOne);
            if (stationOne.retCode != 0) {
                alert(data.retMsg);
                return;
            }
            var allArr = stationOne.data;
            // 上行
            var arr1 = [];
        ...
        getListByRouteId2() {
            let that = this;
            console.log('stationTwo', stationTwo);
            if (stationTwo.retCode != 0) {
                alert(data.retMsg);
                return;
            }
            var allArr = stationTwo.data;
            // 上行
            var arr1 = [];
            for (var i = 0; i < allArr.length; i++) {
                if (allArr[i].stationMark == '0' || allArr[i].stationMark == '1' || allArr[i].stationMark == '2') {
        ...
        adrealInfo: function() {
            ...
            // let runDate = moment(that.runDate, 'YYYY-MM-DD') + ' ' + moment(that.time, 'HH:mm:ss');
            // let firstDate = moment(that.runDate, 'YYYY-MM-DD') + ' ' + moment(that.time, 'HH:mm:ss');
            let runDate = '2023-12-18 07:00:12';
            let firstDate = '2023-12-18 07:00:12';
        ...
        adrealInfo2: function() {
            ...
            // var runDate = moment(that.runDate, 'YYYY-MM-DD') + ' ' + moment(that.time, 'HH:mm:ss');
            // var firstDate = moment(that.runDate, 'YYYY-MM-DD') + ' ' + moment(that.time, 'HH:mm:ss');
            var runDate = '2023-12-18 07:00:12';
            var firstDate = '2023-12-18 07:00:12';
