    ...
    <div
      ...
      <div class="table-page-search-wrapper">
        <a-form @keyup.enter.native="searchQuery">
          ...
          <a-row>
            <a-col :md="24" :sm="24">
                 <a-button style="margin-bottom: 10px" @click="addPermissionRule" v-show="isSuperAdmin" type="primary" icon="plus">添加</a-button>
            </a-col>
          </a-row>
        </a-form>
    ...
    methods: {
      loadData() {
        ...
        getPermissionRuleList(params).then((res) => {
                  if (res.success) {
                    that.dataSource = res.result.result
                    that.isSuperAdmin = res.result.isSuperAdmin
                    that.columns = that.isSuperAdmin ? superAdmincolumns:columns
                  }
                })
      },
      edit(record) {
