      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          ...
          <a-form-item
            ...
            :wrapperCol="wrapperCol"
            label="应用">
            <a-popconfirm title="菜单选择外部应用，【前端组件】建议配置iframe组件：layouts/IframePageView，是否配置？" ok-text="是" cancel-text="否" @confirm="confirmApp"
                          @cancel="cancelApp" :visible="visibleConfirm">
              <a-select :allowClear="true" placeholder="请选择菜单所属应用" style="width: 200px" v-decorator="[ 'appId',validatorRules.app]" :readOnly="disableSubmit" @change="checkComponent">
                <a-select-option href="#" v-for="item in sysApps" :value="item.id">{{item.name}}</a-select-option>
              </a-select>
            </a-popconfirm>
          </a-form-item>
...
<script>
  import {addPermission,editPermission,queryTreeList,getSysAppList} from '@/api/api'
  ...
  export default {
    ...
    methods: {
      ...
      loadApps() {
        ...
        getSysAppList(params).then((res)=>{
          if(res.success){
            console.log('----queryAppList---')
            console.log(res)
            that.sysApps = res.result.records
          }
        });
      },
      add (value) {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.loadTree();
        let fieldsVal = pick(this.model,'name','perms','permsType','component','url','sortNo','menuType','status','platformName','appId');
        this.$nextTick(() => {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.model.route = this.routeSwitch?1:0;
            this.model.keepAlive = this.isKeepalive?1:0;
            this.model.leaf = this.leaf?1:0;
            /*update_begin author:wuxianquan date:20190908 for:获取值*/
            this.model.internalOrExternal = this.internalOrExternal?1:0;
      ...
      checkComponent(value){
        if (value) {
          if (this.form.getFieldValue('component') !== "layouts/IframePageView") {
            this.visibleConfirm = true;
          }
        }
      },
      confirmApp(e){
        this.form.setFieldsValue({component:"layouts/IframePageView"})
