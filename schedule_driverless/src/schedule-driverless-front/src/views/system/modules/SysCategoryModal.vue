<template>
  <a-modal
   ...
   <a-form-item
          ...
          label="所屬平台">
              <g-platform-select placeholder="请选择平台" :triggerChange="true" v-decorator="[ 'platformCode', validatorRules.platformCode]" />
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
       this.$nextTick(() => {
                 this.form.setFieldsValue(pick(this.model,'platformCode','pid','name','code'))
               })
      },
      close () {
        this.$emit('close');
