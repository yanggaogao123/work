<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator" :md="24" :sm="24" style="margin: -46px 0px 10px 2px">
      ...
      <a-button @click="handleAddUserOrg" type="primary" icon="plus">添加已有用户</a-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: "/sys/user/orgUserList",
          edit: "/sys/user/editSysOrgWithUser",
          delete: "/sys/user/deleteUserInOrg",
          deleteBatch: "/sys/user/deleteUserInOrgBatch",
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      batchDel: function () {
        ...
        if (this.selectedRowKeys.length <= 0) {
          ...
          this.$confirm({
            ...
            content: "是否删除选中数据?",
            onOk: function () {
              deleteAction(that.url.deleteBatch, {orgId: that.currentOrgId, userIds: ids}).then((res) => {
                if (res.success) {
      ...
      handleDelete: function (id) {
        ...
        var that = this;
        deleteAction(that.url.delete, {orgId: this.currentOrgId, userId: id}).then((res) => {
          if (res.success) {
            that.$message.success(res.message);
            if (this.selectedRowKeys.length>0){
      ...
      handleEdit: function (record) {
        ...
        this.$refs.modalForm.orgDisabled = true;
      ...
      handleAdd: function () {
        if (this.currentOrgId == '') {
          ...
          this.$refs.modalForm.orgDisabled = true;
          this.$refs.modalForm.userOrgModel.orgIdList = [this.currentOrgId];  //传入一个部门id
