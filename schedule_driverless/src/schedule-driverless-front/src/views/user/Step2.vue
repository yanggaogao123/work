  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          captcha: {rule: [{required: true, message: '请输入验证码!'}, {validator: this.validateCaptcha}]},
          phone: {rule: [{required: true, message: '请输入手机号码/邮箱!'},{validator: this.validatePhone}]},
        },
      }
    },
    computed: {
      ...
      isEmail: function() {
        ...
        return reg.test(this.userList.phone);
      }
    },
    methods: {
      nextStep() {
          ...
          if (!err) {
            if (that.dropList == "0") {
              if (values.captcha == undefined) {
                this.cmsFailed("请输入验证码!");
              } else {
                var params = {}
                params.phone = this.userList.phone;
                params.smscode = values.captcha;
                postAction("/sys/user/verification", params).then((res) => {
                  if (res.success) {
                    console.log(res);
                    var userList = {
      ...
      getCaptcha(e) {
        if (this.userList.phone == null || this.userList.phone == ""){
          this.$message.warn("请输入验证手机号或邮箱")
          return;
        }
        e.preventDefault();
        let that = this;
        ...
        postAction(url, smsParams).then(res => {
          if (!res.success) {
            setTimeout(hide, 1);
            this.cmsFailed(res.message);
          }
