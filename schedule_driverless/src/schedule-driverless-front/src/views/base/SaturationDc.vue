  ...
  <a-card :bordered="false">
		<div class="content">
			<div class="top">
				<div></div>
				<div class="tips">
					<div class="l0"><div class="color-box"></div>低负荷(0-6)</div>
					<div class="l1"><div class="color-box"></div>中负荷(7-10)</div>
					<div class="l2"><div class="color-box"></div>高负荷(10以上)</div>
				</div>
			</div>
			<div class="container">
				<div v-for="staurationItem in staurationData" :key="staurationItem.roadId">
					<div class="station-list">
						<div class="station-item" v-for="item,index in staurationItem.stations" :key="index">
							<div class="up">
								<div :class="{l0: item.upLevel == '0', l1: item.upLevel == '1', l2: item.upLevel == '2'}">{{item.up}}</div>
							</div>
							<div class="down">
								<div :class="{l0: item.downLevel == '0', l1: item.downLevel == '1', l2: item.downLevel == '2'}">{{item.down}}</div>
							</div>
							<div class="title"><div class="title-box">{{item.stationName}}</div></div>
						</div>
...
<script>
  import { getAction } from "@/api/manage";
	import { connection, disConnection, unsubscribe } from '@/utils/sorket'
  ...
  export default {
    ...
    data() {
      return {
				staurationData: [],
        url: {
          getSaturationDc: "/base/saturation/list",
        },
    ...
    destroyed: function () { // 离开页面生命周期函数
      ...
      disConnection(this.stompObj)
    },
    methods: {
      connection() {
				let that = this
        let sendData = {}
				connection(that.stompObj, 'saturation', sendData,function(e) {
