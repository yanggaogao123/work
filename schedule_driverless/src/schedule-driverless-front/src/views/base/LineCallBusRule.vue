<template>
  <a-card :bordered="false">
    ...
    <LineCallBusRule-modal ref="modalForm" @ok="modalFormOk"></LineCallBusRule-modal>
  </a-card>
</template>
...
<script>
  ...
  import LineCallBusRuleModal from './modules/LineCallBusRuleModal'

  export default {
    name: 'LineCallBusRule',
    mixins: [JeecgListMixin, mixinDevice],
    components: {
      GCIOrgTreeSelect, LineCallBusRuleModal
    },
    data() {
      return {
        excelLoading: false,
        ...
        columns: [
          ...
          },
          {
            title: '叫车人数因素判断操作符',
            align: 'center',
            dataIndex: 'callBusCountOperator',
            customRender: function (text) {
              if (text === '1') {
                return '>';
              } else if (text === '2') {
          ...
          },
          {
            title: '起始叫车人数',
            align: 'center',
            dataIndex: 'callBusCountBegin'
          },
          {
            title: '终止叫车人数',
            align: 'center',
            dataIndex: 'callBusCountEnd'
          },
          {
            title: '因素关系',
            align: 'center',
            dataIndex: 'factorLogical',
            customRender: function (text) {
              if (text === '1') {
                return '和';
              } else if (text === '2') {
          ...
          },
          {
            title: '距离站数因素判断操作符',
            align: 'center',
            dataIndex: 'stationCountOperator',
            customRender: function (text) {
              if (text === '1') {
                return '>';
              } else if (text === '2') {
          ...
          },
          {
            title: '起始站数',
            align: 'center',
            dataIndex: 'stationCountBegin'
          },
          {
            title: '终止站数',
            align: 'center',
            dataIndex: 'stationCountEnd'
          },
          {
            title: '备注',
            align: 'center',
        ...
        url: {
          list: '/base/lineCallBusRule/list',
          delete: '/base/lineCallBusRule/delete',
          export: '/base/lineCallBusRule/exportLineCallBusRule',
    ...
    methods: {
      ...
      async exportCsv() {
        ...
        await downloadFileAwait(this.url.export, '线路叫车规则管理.csv', params)
      ...
      async exportExcel() {
        ...
        let params = Object.assign(exportType, this.queryParam)
        console.log('params', params)
        await downloadFileAwait(this.url.export, '线路叫车规则管理.xlsx', params)
