...
public class ScheduleGenerateTest {
	...
	public RouteSchedule generateDriverless() {
		if (routeSchedule == null) {
			routeSchedule = new RouteSchedule(scheduleParam);
		}
		scheduleParam.restSubSectionPassengerAll();
	...
	private Trip getTripByStation(Bus bus, Integer direction, Date tripBeginTime, Long firstRouteStaId, Date tripBeginTimeNext, Long firstStationId, Long lastStationId) {
		if (tripBeginTimeNext != null && tripBeginTime.after(tripBeginTimeNext)) {
			return null;
		}
		List<ScheduleParamsDrRouteSub> list = scheduleParam.getScheduleParamsDrRouteSubList();
		List<ScheduleParamsDrRouteSub> newList = new ArrayList<>();
		for (ScheduleParamsDrRouteSub scheduleParamsDrRouteSub : list) {
			if ((direction == null || direction.equals(scheduleParamsDrRouteSub.getDirection())) &&
					(firstStationId == null || firstStationId.equals(scheduleParamsDrRouteSub.getFirstStationId())) &&
//					(lastStationId == null || lastStationId.equals(scheduleParamsDrRouteSub.getLastStationId())) &&
					(firstRouteStaId== null || firstRouteStaId.equals(scheduleParamsDrRouteSub.getFirstRouteStaId())) &&
					(lastStationId != null || scheduleParamsDrRouteSub.getNextFirstRouteStaId() != null)) {
				newList.add(scheduleParamsDrRouteSub);
			}
		}
		list = newList;
		if (direction == null && !list.isEmpty()) {
			direction = list.get(0).getDirection();
		}
		Trip tripNew = null;
		if (list != null) {
			List<Trip> tripListAll = new ArrayList<>();
			for (ScheduleParamsDrRouteSub routeSub : list) {
				...
				if (tripBeginTimeNext == null || !trip.getTripEndTime().after(tripBeginTimeNext)) {
					tripListAll.add(trip);
					if (lastStationId == null || lastStationId.equals(routeSub.getLastStationId())) {
			...
			if (tripNew == null && lastStationId != null) {
				Collections.sort(tripListAll, Comparator.comparing(Trip::getTripEndTime).reversed());
				for (Trip trip : tripListAll) {
					if (trip.getFirstRouteStaIdNext() != null) {
						Trip tripNext = getTripByStation(bus, null, trip.getNextObuTimeMin(), trip.getFirstRouteStaIdNext(), tripBeginTimeNext, null, lastStationId);
						if (tripNext != null) {
							tripNew = trip;
							break;
						}
			...
			for (ScheduleParamsDrInoutTime inoutTime : drBus.getInoutTimeList()) {
				...
				if (inoutTime.getOutTime() == null) {
					if (inoutTime.getInTime() == null) {
						...
						if (inoutTime.getInTime().after(tripBeginTime)) {
							tripBeginTime = inoutTime.getInTime();
						}
						trip = getTripByPassenger(bus, null, tripBeginTime, null, inoutTime.getInStationId());
	...
	private Trip getTripByPassenger(Bus bus, Integer direction, Date tripBeginTime, Long firstRouteStaId, Long firstStationId) {
		...
		for (ScheduleParamsDrRouteSub scheduleParamsDrRouteSub : list) {
			if ((direction == null || direction.equals(scheduleParamsDrRouteSub.getDirection())) &&
					(firstStationId == null || firstStationId.equals(scheduleParamsDrRouteSub.getFirstStationId())) &&
					(firstRouteStaId== null || firstRouteStaId.equals(scheduleParamsDrRouteSub.getFirstRouteStaId())) &&
					scheduleParamsDrRouteSub.getNextFirstRouteStaId() != null) {
				newList.add(scheduleParamsDrRouteSub);
			}
		}
		list = newList;
