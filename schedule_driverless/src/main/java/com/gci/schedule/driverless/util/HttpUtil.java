	...
	public static String queryString(final HttpRequestBase requestBase) {
		...
		try {
			...
			if (status == 200) {
				...
				log.error(String.valueOf(new StringBuffer("链接出错，url：")
						.append(requestBase.getURI()).append(",错误代码")
						.append(status)));
			}
		} catch (ClientProtocolException e1) {
			log.error(String.valueOf(new StringBuffer("ClientProtocolException,url：")
					.append(",错误代码")), e1);
		} catch (IOException e1) {
			log.error(String.valueOf(new StringBuffer("IOException,url：")
					.append(requestBase.getURI())), e1);
		} finally {
			try {
				if (resq != null) {
					resq.close();
